...
- **돌봄기기 사용감지 센서(ESP32 압력/FSR) 연동 — 수신 + 대시보드 표시** (4번째 기기 유형)
  ...
  - 이벤트 기반 센서 특성 반영: 안 쓰는 동안 조용한 게 정상이므로 10분 무소식 규칙을
    적용하지 않고, `FSR_STALE_SEC`(기본 6시간)을 넘겨야 `센서 무응답`.
    경과 판정은 문자열 시각이 아니라 epoch(`last_seen_ts`)로 계산해 서버 시간대와 무관하게 동작
  - ⚠️ `센서 무응답`을 `미사용`과 별도 상태로 둔 이유 — 배터리가 나가 '사용 중'에서 멈춘
    센서가 화면에 `미사용`으로 보이면 안 쓰는 건지 고장인지 구분할 수 없다
  - 생존신고(heartbeat)가 마지막으로 들어와도 직전 사용/미사용 상태를 잃지 않도록
    `get_latest_states` 에서 확정 상태를 따로 보존해 합침
  - ⚠️ **기기 종류를 SN 모양으로 추측하지 않는다** — ESP32 의 MAC(`ECE334450058`)도
  ...
  - `deviceId` 가 MAC 형태면 라닉스·McKare 와 동일하게 '구분자 없는 대문자'로 정규화 —
    펌웨어가 `EC:E3:34:...` 로 보내다 `ece334...` 로 바꿔도 카드가 갈라지지 않음
  ...
  - 대시보드: 기존/신규(v2) 양쪽에 **FSR 발판 섹션** 추가. 심박·호흡 칸 대신
    발판 상태(눌림/해제) + 배터리 + 직전 눌림 지속시간 표시. 헤더 요약에 `FSR N대`
