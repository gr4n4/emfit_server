...
**결론 두 가지:**
1. **`user_summary` 에서 걸음·거리를 읽을 수 없다.** 필드 자체가 없는 날이 대부분이다
   (`Total Steps` 빈칸 = 키 부재, 0이 아님). → `get_steps_data()` 의 **15분 버킷을 직접 합산**해야 한다
   (`health_panels.steps_hourly()` 가 이미 그렇게 `total` 을 계산한다).
2. **돌봄받는 분(0003·0004)에게 걸음수는 의미가 없다.** 카드에 걸음수를 고정 칸으로 두면 항상 `0`·`-` 이
   뜬다. 대신 **활동레벨 분포(sedentary / sleeping / active)** 가 두 유형 모두에게 의미 있는 지표다 (§7).
...
- **걸음수를 고정 칸으로 두지 않는다** — 돌봄받는 분은 항상 0이다(§3 실측). 대신 **활동레벨 기반**으로:
  | 칸 | 값 | 출처 |
  |---|---|---|
  | ❤️ 안정 | 안정시 심박 | `user_summary.restingHeartRate` |
  | 🛋 좌식 / 🚶 활동 | 좌식 시간, 또는 걸음수가 있으면 걸음수 | `get_steps_data()` 15분 버킷 집계 |
  | 😴 수면 | 수면 시간(또는 점수) | `get_sleep_data()` |
- 걸음수가 실제로 잡히는 계정(0005·0006)은 `🚶 걸음`으로, 0인 계정은 `🛋 좌식 시간`으로 **자동 전환**하면
  한 렌더러로 두 유형을 다 처리할 수 있다. (FSR 배터리를 값 없을 때 빈칸 처리한 방식과 동일한 접근)
...
| **1-b. 원본 JSON 확보** | 같은 호출의 **응답 JSON 원본**을 파일로 저장 | 파서 작성 근거 | 없음 |
...
**Garmin 의 `sleeping` 활동레벨 버킷으로 EMFIT 수면 판정을 교차 검증할 수 있다.**
0003 은 30일간 `sleeping` 621버킷(≈155시간, 하루 5.2시간), 0004 는 511버킷이 잡혔다. 손목에서 독립적으로
측정한 값이므로 침대 센서와 대조가 가능하다.

이게 의미 있는 이유: MANUAL §9 의 알려진 한계 — **EMFIT-DEMO-04(뇌성마비 대상자)의 EMFIT 수면 단계 분류가
실패해 REM/깊은수면이 대부분 0** — 을 다른 측정 경로로 보완할 여지가 생긴다. 0003·0004 도 거동이 불편한
분들이라 같은 문제를 겪을 가능성이 있고, 그렇다면 "침대 센서가 수면을 못 읽는 대상자에게 워치가 답을
줄 수 있는가"가 실증 질문이 된다. (지금 당장 구현할 것은 아니고, 데이터가 쌓인 뒤 분석 과제)
