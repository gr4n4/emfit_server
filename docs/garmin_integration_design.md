...
| 0004 | **0** | **30일 전부 빈칸** | sedentary 2241 · sleeping 511 · **`wheelchair_pushing` 9** | **휠체어 사용자** |
| 0005 | 180,095 | 24/30일 | sedentary 2607 · active 187 · highlyActive 26 | 보행 활발 |
| 0006 | 174,830 | 26/30일 | sedentary 2600 · active 87 · highlyActive 50 | 보행 활발 |

### ⭐ 원인 규명: `totalSteps` 와 `totalPushes` 는 상호 배타다 (0004·0005 원본 비교)

원본 JSON 을 받아 비교한 결과, "걸음을 읽을 수 없다"가 아니라 **휠체어 사용자는 걸음 대신 밀기(pushes)로
집계된다**는 것이 원인이었다. `user_summary` 는 두 유형 모두 값을 제대로 준다.

| `user_summary` 필드 | 0004 (휠체어) | 0005 (보행) |
|---|---|---|
...
2. 지표 가용성은 계정·기기마다 다르다 (`averageSpo2` 는 0004 만 있음). **없는 값은 빈칸으로 두고
   카드·CSV 에서 자동으로 빠지게** 한다 — FSR 배터리를 값 없을 때 숨기는 방식과 동일.
3. ~~"user_summary 에서 걸음·거리를 읽을 수 없어 버킷을 합산해야 한다"~~ → **철회.** 0004 만 보고 내린
...
Garmin 은 **같은 날짜를 폴링할 때마다 그날 전체를 다시 준다.** 실측 사례(0005, 09-14 오전):
...
- 가운데 칸은 **한 렌더러로 두 유형을 자동 처리**한다 (§3 — 두 필드는 상호 배타):
  ...
  | 조건 | 표시 | 실측 예 |
  |---|---|---|
  | `totalSteps` 있음 | `🚶 걸음` | 0005 → 7,040 |
  | `totalSteps` 없고 `totalPushes` 있음 | `🦽 밀기` | 0004 → 19 |
...
|---|---|---|---|
| example-account-03 | 20일 | 거동 거의 없음 (걸음 0) | A시설2 박O희님 |
| example-account-04 | 29일 | **휠체어 사용** (`wheelchair_pushing`) | A시설1 사용자-A님 |
| example-account-05 | 25일 | **보행 활발** (18만 걸음/30일) | **미지정 — 확인 필요** |
| example-account-06 | 26일 | **보행 활발** (17만 걸음/30일) | **미지정 — 확인 필요** |
...
| 계정 | `/devices` 등록명 | 상태 |
|---|---|---|
| example-account-04 | **A시설1 사용자-A님** | 🟢 정상 (밀기 19 — 휠체어) |
| example-account-05 | **실증그룹-B_사용자-B님** | 🟢 정상 (걸음 7,040) |
| example-account-03 | (미지정) | 🔴 **09-10 부터 착용 중단** — 09-09 까지는 하루 720건 정상 |
| example-account-06 | (미지정) | 💤 비활성 — 최근 데이터 없음 |

> ~~가설: 0005·0006 은 A시설의 돌봄자(요양보호사·가족)일 것이다~~ → **틀렸다.**
> 0005 는 A시설가 아니라 WIM 중장기 대상자였다. 걸음수가 많다는 것만으로 돌봄자로 단정한
...
### ✅ 2026-09-14 PoC 결과: oauth1 정상 (0004 계정)
...
이게 의미 있는 이유: MANUAL §9 의 알려진 한계 — **EMFIT-DEMO-04(뇌성마비 대상자)의 EMFIT 수면 단계 분류가
실패해 REM/깊은수면이 대부분 0** — 을 Garmin 이 보완할 수 있음이 데이터로 확인됐다. 휠체어를 쓰는
0004 에게 깊은수면 30분·REM 24분이 정상적으로 산출된다.
